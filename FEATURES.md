...
- **多类型支持** - 文本、图片、文件、HTML、RTF 等多种内容类型
...
- **截图识别** - 支持截取屏幕区域进行文字识别
- **快捷键触发** - 可通过快捷键快速启动 OCR 截图（默认 F6）
- **识别结果管理** - 识别结果可编辑、复制，支持多种 OCR 引擎配置
- **独立设置面板** - 在设置中单独配置 OCR 相关参数
...
- **多引擎支持** - 可配置不同的翻译服务引擎
- **翻译结果页** - 独立的翻译结果展示页面
- **独立设置面板** - 在设置中配置翻译引擎和参数

## 云端同步

- **WebDAV 同步** - 支持通过 WebDAV 协议同步剪贴板数据
- **自动同步** - 后台自动同步，无需手动操作
- **代理支持** - 支持 HTTP/SOCKS 代理，适配各种网络环境
- **独立设置面板** - 在设置中配置 WebDAV 服务器地址、账号等
...
- **内容大小限制** - 单条内容最大大小可配置（文本、图片、文件、视频分别可设）
...
- **一键开启** - 在设置中启用游戏模式，暂停剪贴板监控
- **减少干扰** - 游戏或全屏应用时避免剪贴板弹窗干扰

## 系统集成

- **系统托盘** - 左键切换窗口、右键菜单（设置、重启、退出）
- **托盘图标开关** - 可在设置中控制托盘图标的显示/隐藏
