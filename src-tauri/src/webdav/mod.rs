...
static SYNC_OPERATION_RUNNING: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

pub struct SyncOperationGuard;

pub fn try_begin_sync_operation() -> Option<SyncOperationGuard> {
    SYNC_OPERATION_RUNNING
        ...
        .ok()
        .map(|_| SyncOperationGuard)
}

impl Drop for SyncOperationGuard {
    fn drop(&mut self) {
        SYNC_OPERATION_RUNNING.store(false, std::sync::atomic::Ordering::Release);
    }
}
...
/// media_map.json 的紧凑传输结构。
///
/// 内部仍使用 Vec<MediaEntry>，传输时按 device_id 分组，避免每个媒体条目重复写入设备标识。
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
struct MediaMapSyncData {
    devices: Vec<MediaDeviceSyncData>,
}

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
struct MediaDeviceSyncData {
    device_id: String,
    entries: Vec<MediaMapEntry>,
}
...
impl From<&MediaEntry> for MediaMapEntry {
...
impl MediaMapEntry {
    fn into_media_entry(self, device_id: String) -> MediaEntry {
        MediaEntry {
            hash: self.hash,
            ext: self.ext,
...
fn serialize_media_map(map: &[MediaEntry]) -> Result<String, String> {
    let mut grouped = std::collections::BTreeMap::<String, Vec<MediaMapEntry>>::new();
    for entry in map {
        grouped
            .entry(entry.device_id.clone())
            .or_default()
            .push(MediaMapEntry::from(entry));
    }

    let payload = MediaMapSyncData {
        devices: grouped
            .into_iter()
            .map(|(device_id, entries)| MediaDeviceSyncData { device_id, entries })
            .collect(),
    };
    serde_json::to_string_pretty(&payload).map_err(|e| e.to_string())
}

fn parse_media_map(json: &str) -> Result<Vec<MediaEntry>, String> {
    let payload: MediaMapSyncData = serde_json::from_str(json)
    ...
    Ok(payload
        ...
        .flat_map(|device| {
            ...
            device
                .entries
                .into_iter()
                .map(move |entry| entry.into_media_entry(device_id.clone()))
        })
        .collect())
}
...
pub fn calc_max_byte_size(max_size_kb: u64) -> i64 {
    if max_size_kb > 0 {
        max_size_kb.saturating_mul(1024).min(i64::MAX as u64) as i64
    } else {
        i64::MAX
    }
}
...
pub fn export_sync_data_to_file(
    ...
    let result = (|| {
        let file = OpenOptions::new()
            ...
            .write(true)
            .open(&path)
            .map_err(|error| format!("创建同步临时文件失败: {}", error))?;
        ...
        if options.sync_settings {
            ...
            if let Ok(all_settings) = settings_repo.get_all() {
                ...
                serde_json::to_writer(&mut zip, &all_settings)
        ...
        zip.start_file("items.json", zip_options)
            .map_err(|e| e.to_string())?;
        serde_json::to_writer(&mut zip, &items).map_err(|e| format!("序列化条目失败: {}", e))?;
        ...
        if let Ok(tags_data) = tag_repo.export_tags_sync_data() {
            if !tags_data.tags.is_empty() {
                zip.start_file("tags.json", zip_options)
                    .map_err(|e| e.to_string())?;
                serde_json::to_writer(&mut zip, &tags_data)
                    .map_err(|e| format!("序列化标签失败: {}", e))?;
                info!(
                    "标签同步导出: {} 个标签, {} 条关联",
                    tags_data.tags.len(),
                    tags_data.association_count()
                );
            }
        }
        ...
        if !media_map.is_empty() {
            zip.start_file("media_map.json", zip_options)
                .map_err(|e| e.to_string())?;
            let media_map_json = serialize_media_map(&media_map)
                .map_err(|e| format!("序列化媒体映射失败: {}", e))?;
            std::io::Write::write_all(&mut zip, media_map_json.as_bytes())
                .map_err(|e| format!("写入媒体映射失败: {}", e))?;
...
pub fn import_sync_data(
    ...
    if let Ok(mut entry) = archive.by_name("media_map.json") {
        ...
        result.media_map = parse_media_map(&json)?;
    }
...
pub fn plan_media_downloads(
    ...
    media_map
        ...
        .filter(|entry| {
            ...
            let referenced_locally =
                referenced.contains(&entry.local_path) || referenced.contains(&target_string);
...
/// 以单个后台 worker 顺序处理媒体，避免一次同步同时启动多个磁盘/网络任务。
pub fn sync_media_entries_sequential(
    config: &WebDavConfig,
    data_dir: &Path,
    local_entries: &[MediaEntry],
    download_entries: &[MediaEntry],
) {
    for (media_type, label) in [
        ...
        let local: Vec<MediaEntry> = local_entries
            .iter()
            .filter(|entry| entry.media_type == media_type)
            .cloned()
            .collect();
        let missing: Vec<MediaEntry> = download_entries
            .iter()
            .filter(|entry| entry.media_type == media_type)
            .cloned()
            .collect();
        ...
        if !local.is_empty() {
            match upload_media_files(config, &local, data_dir) {
                Ok((uploaded, skipped, _)) => {
                    info!("{}上传: {} 个，{} 个跳过", label, uploaded, skipped)
                }
                Err(error) => info!("{}上传失败: {}", label, error),
            }
        }
        if !missing.is_empty() {
            match download_missing_media(config, &missing, data_dir) {
                Ok(downloaded) => {
                    if downloaded > 0 {
                        info!("{}下载: {} 个", label, downloaded);
                    }
                }
                Err(error) => info!("{}下载失败: {}", label, error),
            }
        }
    }
}
...
pub fn upload_media_map(
    ...
    // 上传合并后的映射
    let json = serialize_media_map(&map)?;
            ...
            loop {
                ...
                if enabled && auto_sync {
                    ...
                    if let Some((config, options)) = load_config_and_options(&db) {
                        ...
                        match export_sync_data_to_file(&db, &data_dir, &options) {
                            Ok((zip_path, _size)) => {
                                let upload_result =
                                    upload_sync_file(&config, &zip_path, SYNC_FILENAME);
                        ...
                        let need_media =
                            (options.sync_image || options.sync_files || options.sync_video)
                                && cycle_count % 3 == 0;
                        if need_media
                            && !MEDIA_SYNC_RUNNING.load(std::sync::atomic::Ordering::Relaxed)
                        {
                            ...
                            // Serialize media work through one worker to cap disk/network concurrency.
                            if local_map.is_empty() && needed.is_empty() {
                                MEDIA_SYNC_RUNNING
                                    .store(false, std::sync::atomic::Ordering::Relaxed);
                                if fixed > 0 {
                                ...
                                let local_entries = local_map.clone();
                                let download_entries = needed;
                                let app_handle = app.clone();
                                let spawn_result = std::thread::Builder::new()
                                    .name("webdav-sync-media".into())
                                    .spawn(move || {
                                        sync_media_entries_sequential(
                                            &cfg,
                                            &dir,
                                            &local_entries,
                                            &download_entries,
                                        ...
                                        MEDIA_SYNC_RUNNING
                                            .store(false, std::sync::atomic::Ordering::Relaxed);
                                        emit_webdav_media_ready(&app_handle);
                                    });
                                if let Err(error) = spawn_result {
                                    info!("media sync worker start failed: {}", error);
                                    MEDIA_SYNC_RUNNING
                                        .store(false, std::sync::atomic::Ordering::Relaxed);
                                    if fixed > 0 {
                                        emit_webdav_media_ready(&app);
...
/// 流式上传本地临时文件，避免把完整 ZIP 再复制到 reqwest 的内存缓冲区。
pub fn upload_sync_file(config: &WebDavConfig, path: &Path, filename: &str) -> Result<(), String> {
    use std::fs::File;

    let file = File::open(path).map_err(|error| format!("打开同步临时文件失败: {}", error))?;
    let size = file
        .metadata()
        .map_err(|error| format!("读取同步临时文件大小失败: {}", error))?
        .len();
    let client = build_client(config)?;
    let auth = basic_auth(&config.username, &config.password);
    let base_url = normalize_url(&config.url, &config.remote_dir);
    ...
    ensure_remote_dir(&client, &config.url, &config.remote_dir, &auth)?;
    let file_url = format!("{}{}", base_url, filename);
    info!("WebDAV 流式上传: {} ({} bytes)", file_url, size);
...
mod tests {
    ...
    #[test]
    fn media_map_json_groups_entries() {
        let mut first = media_entry("image", r"C:\one.png", "hash-1");
        first.file_name = "one.png".to_string();
        first.size = 12;
        let mut second = media_entry("file", r"C:\two.txt", "hash-2");
        second.file_name = "two.txt".to_string();
        second.size = 34;

        let json = serialize_media_map(&[first.clone(), second.clone()]).unwrap();
        assert!(json.contains("\"devices\""));
        assert_eq!(json.matches("\"device_id\"").count(), 1);

        let parsed = parse_media_map(&json).unwrap();
        assert_eq!(parsed.len(), 2);
        assert_eq!(parsed[0].device_id, "device");
        assert_eq!(parsed[0].file_name, "one.png");
        assert_eq!(parsed[1].size, 34);
    }

    #[test]
    fn legacy_media_map_array_is_rejected() {
        let legacy_json = r#"[{"hash":"hash-1","ext":"png","media_type":"image","local_path":"C:\\one.png"}]"#;
        assert!(parse_media_map(legacy_json).is_err());
    }
    ...
    fn rewrite_maps_remote_file_to_staged_target_and_marks_it_missing() {
        ...
        assert_eq!(
            paths[0],
            r"D:\\ElegantClipboard\staged\webdav\abc123_report.txt"
        );
    ...
    fn importability_requires_media_mapping_for_missing_files() {
        ...
        assert!(!item_importable_for_sync(
            &item,
            &empty,
            i64::MAX,
            i64::MAX,
    ...
    fn build_media_map_keeps_same_content_at_distinct_paths() {
        let root =
            std::env::temp_dir().join(format!("elegant-clipboard-webdav-{}", uuid::Uuid::new_v4()));
